...
`center`、`right`、`note` 容器是一种简单的 Markdown 容器（不改变内容，只给内容加样式），Teek 支持定义类似的容器，具体请看 [markdownContainers 配置项](/reference/plugin-config#markdowncontainers)。
...
Demo 容器用于展示编写的 Vue 组件效果，且支持查看源代码、复制源代码、去 `Github` 编辑、去 `Playground` 编辑功能：
