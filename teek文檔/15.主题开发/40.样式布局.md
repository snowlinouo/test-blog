...
在 `components` 目录下，Teek 会给每个组件生成对应的样式文件，文件名与组件名相同（首字母小写）。
...
命名空间应该是一个变量，这样只需要修改该变量的值，那么所有的 `class` 以及样式都不会失效，如在 Element Plus 的命名空间文件里修改 `el` 为 `tk`，那么所有的 `class` 都变为 `tk` 开头，且样式不会失效。
...
通过 SCSS Module API 可以将 `$namespace` 变量导出到 `js` 或 `ts` 文件里，在 `styles/module/namespace.module.scss` 文件导出 `$namespace`：
...
定义一个 Hooks 文件 [useNamespace.ts](https://github.com/Kele-Bingtang/vitepress-theme-teek/blob/master/vitepress-theme-teek/src/hooks/useNamespace.ts) 来封装命名空间和 BEM 规范，[命名空间](#命名空间) 和 [BEM 规范](#什么是-bem) 在上文已经介绍过了。
...
定义 SCSS 文件 [bem.scss](https://github.com/Kele-Bingtang/vitepress-theme-teek/blob/master/vitepress-theme-teek/src/styles/mixins/bem.scss) 来封装命名空间和 BEM 规范，
